...
class DataAnalyzer:
    ...
    def create_all_graphs(self):
        ...
        if not os.path.exists('2analysis_graphs'):
            os.makedirs('2analysis_graphs')
...
class Game:
    ...
    def _save_current_data(self):
        """Save current simulation state data"""
        current_data = []

        # Sample current creatures' performance
        for creature in [self.top, self.bottom]:
            data = self.analyzer.calculate_performance(
                creature.L, creature.freq, self.rho, self.mu, creature.propulsion
            )
            data['environment'] = self.current_env
            data['scale'] = 'bacteria' if creature.is_bacteria else 'fish'
        ...
        filename = f'analysis_graphs/current_simulation_{self.current_env}_{self.scenario}.csv'
        df.to_csv(filename, index=False)

        self.analysis_status = f"Current data saved to {filename}"
        self.analysis_timer = pygame.time.get_ticks()
