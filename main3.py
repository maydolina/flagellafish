...
class EnhancedGame:
    ...
    def handle_events(self):
        ...
        adjustment_speed = 1.015
        if keys[pygame.K_UP]:
            self.creature.adjust_size(adjustment_speed)
            self.active_control = "Size"
        if keys[pygame.K_DOWN]:
            self.creature.adjust_size(1 / adjustment_speed)
            self.active_control = "Size"
        if keys[pygame.K_RIGHT]:
            self.creature.adjust_frequency(adjustment_speed)
            self.active_control = "Frequency"
        if keys[pygame.K_LEFT]:
            self.creature.adjust_frequency(1 / adjustment_speed)
            self.active_control = "Frequency"
        if keys[pygame.K_v]:
            self.viscosity = min(self.config.MAX_VISCOSITY, self.viscosity * adjustment_speed)
            self.active_control = "Viscosity"
        if keys[pygame.K_b]:
            self.viscosity = max(self.config.MIN_VISCOSITY, self.viscosity / adjustment_speed)
            self.active_control = "Viscosity"
